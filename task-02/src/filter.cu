...
Filter::Filter(int filter_dim, int channels)
...
void Filter::set_filter(float* input_filter) {
...
void Filter::generate_gaussian_blure_filter(int size = 3) {
    if (size == 3 && size == dim) {
        std::memset(h_filter, 0x0, bytes);
        int block_size = dim * dim;
